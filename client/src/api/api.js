...
const instance = axios.create({
  baseURL: "https://booking-app-licenta.herokuapp.com/:5000/",
});
...
export const authAPI = {
  ...
  async confirmRegistration(confirmRegisterToken) {
    const resp = await instance.post(
      `api/auth/confirmRegister/${confirmRegisterToken}`,
      {}
    );
    return resp.data;
  },
...
export const companyAPI = {
  ...
  async addService({ services, companyID, ...props }) {
    try {
      const resp = await instance.post(
        `api/companies/addService/${companyID}`,
        {
          services: services,
        },
        {
  ...
  async editServices({ services, companyID, servicesID, ...props }) {
    try {
      const resp = await instance.post(
        `api/companies/editServices/${companyID}`,
        {
          services: services,
  ...
  async deleteService({ serviceID, companyID }) {
    try {
      const resp = await instance.delete(
        `api/companies/deleteService/${companyID}/${serviceID}`,
        {
          headers: {
            "auth-token": authToken.getToken(),
          },
...
export const guestAPI = {
  async makeReservation({ companyID, serviceID, ...userData }) {
    try {
      const resp = await instance.post(
        `api/companies/makeReservation/${companyID}/${serviceID}`,
        {
          ...userData,
        }
      );
