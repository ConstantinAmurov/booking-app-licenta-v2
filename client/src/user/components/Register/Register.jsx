...
const Register = (props) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    surname: Yup.string("Please enter a valid name").required(
      "This field is required"
    ),
  ...
  const handleSubmit = async (value, { resetForm, setErrors }) => {
    dispatch(registerNewUserThunk(value, setErrors))
      .then((resp) => {
  ...
  const responseFacebook = (response) => {
    ...
    if (accessToken) dispatch(loginfacebookThunk(accessToken, userID));
  };
