    ...
    if (findCompanie) {
      findCompanie.services = findCompanie.services.map((service) => {
        ...
        if (searchServiceIndex !== -1) {
          services[searchServiceIndex].periods.byGuests =
            service.periods.byGuests;
...
router.post("/makeReservation/:companyID/:serviceID", async (req, res) => {
  ...
  if (findCompanie) {
    ...
    debugger;
    const reservMessage = `Hello ${lastName},<br> You got a new message from <b>Booking App</b>:<br> Thank you for booking your service with us!! Your reservation for company: ${
      findCompanie.name
    } on ${new Date(time)
      .toString()
      .split(" ")
      ...
      .split(":")
      .slice(0, 2)
      .join(":")}. please find the details below`;
    try {
      await sendMail({
        to: email,
        subject: "New message from Booking App",
        text: reservMessage,
      });
    } catch (err) {
      res.status(400).json({
